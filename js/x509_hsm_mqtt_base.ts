 ...
 * @private
 */
export class X509HsmMqttBase extends EventEmitter {
    ...
    connect(config: MqttBaseTransportConfig, done: (err?: Error, result?: any) => void): void {
        ...
        if ((!config) ||
            ...
            (!config.sharedAccessSignature &&
                (!config.x509 || !config.x509.cert || !config.x509.privateKeyEngine || !config.x509.privateKeyIdentifier))) {
    ...
    private _connectClient(callback: (err?: Error, connack?: any) => void): void {
        ...
        if (this._config.sharedAccessSignature) {
            ...
            options.privateKeyEngine = this._config.x509.privateKeyEngine;
            options.privateKeyIdentifier = this._config.x509.privateKeyIdentifier;
        }
...
interface IClientOptionsV2 extends IClientOptions {
...
export interface X509V2 extends X509 {
    privateKeyEngine?: string;
    privateKeyIdentifier?: string;
}
